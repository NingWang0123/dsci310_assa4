...
Additionally, note that in Table 1 \@ref(tab:horse-std) we consider the sample standard deviation of the number of horses during the same time span as Figure1 \@ref(fig:horse-1).

```{r horse-2, echo=FALSE, out.width="50%",  fig.cap = "Horse populations for the province with the largest std"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

Figure 2 \@ref(fig:horse-2): Horse populations for the province with the largest standard deviation

In Figure 2 \@ref(fig:horse-2) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
